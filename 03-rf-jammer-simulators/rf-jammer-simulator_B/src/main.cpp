...
void loop() {
  ...
  u8g2.clearBuffer();
  u8g2.drawStr(0, 10, "AGITATOR 2 ACTIVE");
  ...
  if (currentMode == 0) {
    ...
    radio.transmit("FLOODING_CHANNEL_SAOIHFIOSHFOSINOKS");
    delay(9); // Tiny gap
    ...
    // No delay needed, loop keeps it active
    ...
    radio.setPreambleLength(53000); 
    
    // 2. Start transmitting in BACKGROUND (Non-blocking)
    radio.startTransmit("BREAKING_RECEIVERS_WITH_LONG_PREAMBLE"); 
    ...
    float startFreq = 867;
    float endFreq = 869;
    float step = 0.0025; // 50kHz steps (smoother)
    ...
    for (float f = startFreq; f <= endFreq; f += step) {
        ...
        delay(75); // 4. Dwell time (50ms paints a nice solid block on SDRangel)
